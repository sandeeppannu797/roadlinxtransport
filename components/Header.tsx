...
/*
 * Global header, ported verbatim from legacy partials/header.php.
 * Behaviour ported from legacy assets/js/site.js: sticky scroll state,
 * active-nav highlighting, mobile drawer. Dropdown menus are CSS hover.
 */
...
/* Featured menu links, verbatim from legacy header.php. */
const menuServices = [
  { href: '/services/general-freight-transport', title: 'General Freight', sub: 'Everyday palletised & non-palletised loads', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M14 18V6a1 1 0 0 0-1-1H2a1 1 0 0 0-1 1v11a1 1 0 0 0 1 1h1"/><path d="M14 9h4l3 3v5a1 1 0 0 1-1 1h-1"/><circle cx="7" cy="18" r="2"/><circle cx="17" cy="18" r="2"/></svg> },
  { href: '/services/full-truck-load-ftl', title: 'Full Truck Load (FTL)', sub: 'Dedicated, direct point-to-point runs', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8h4l3 3v5h-7z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg> },
  { href: '/services/taxi-truck-hire', title: 'Taxi Truck Hire', sub: 'On-demand utes to 12-tonne trucks', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M5 17H3V6a1 1 0 0 1 1-1h11v12h-4"/><path d="M15 8h4l3 4v5h-4"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg> },
  { href: '/services/tail-lift-tailgate-delivery', title: 'Tail-Lift Delivery', sub: 'Sites with no forklift', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M3 5h11v11H3z"/><path d="M14 9h4l3 3v4h-7"/><path d="M2 20h10"/><circle cx="18" cy="18" r="2"/></svg> },
  { href: '/services/hot-shot-same-day-freight', title: 'Hot Shot & Same-Day', sub: 'Urgent, time-critical freight', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2 3 14h9l-1 8 10-12h-9z"/></svg> },
  { href: '/services/semi-trailer-hire', title: 'Semi-Trailer Hire', sub: '22 & 24-pallet semis with a driver', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="5" width="16" height="11"/><path d="M17 9h4l2 3v4h-6z"/><circle cx="6" cy="18" r="2"/><circle cx="19" cy="18" r="2"/></svg> },
];
...
function navSection(pathname: string): string {
  ...
  return seg; // services, locations, fleet, industries, blog, about, …
}
  ...
  return (
    <>
      <div className="site-topbar">
        <div className="container">
          <div className="topbar-left">
            <a className="tb-item" href="tel:0731797072" aria-label="Call Road Linx Transport">
              <PhoneIcon />
              07 3179 7072
            </a>
          ...
          <div className="topbar-right">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 6v6l4 2"/><circle cx="12" cy="12" r="10"/></svg>
      ...
      <header className={`site-header${scrolled ? ' is-scrolled' : ''}`}>
        <div className="container">
          <Link className="brand" href="/" aria-label="Road Linx Transport — home">
            <img src="/assets/img/roadlinx-logo.png" alt="Road Linx Transport logo" width={180} height={120} />
          </Link>
          ...
          <button className="nav-toggle" aria-label="Open menu" aria-controls="mobileNav" onClick={() => setDrawerOpen(true)}>
      ...
      <div className={`mobile-nav${drawerOpen ? ' is-open' : ''}`} id="mobileNav">
        <div className="mn-scrim" onClick={() => setDrawerOpen(false)}></div>
        <div className="mn-panel" onClick={(e) => { if ((e.target as HTMLElement).closest('a')) setDrawerOpen(false); }}>
          <div className="mn-head">
            <img src="/assets/img/roadlinx-logo.png" alt="Road Linx Transport" style={{ height: 40, width: 'auto' }} />
            <button className="mn-close" aria-label="Close menu" onClick={() => setDrawerOpen(false)}>
              <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18M6 6l12 12"/></svg>
            </button>
          </div>
          <Link href="/">Home</Link>
          <details>
